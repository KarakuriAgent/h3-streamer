...
- **版権もの**：実在のアニメ・ゲーム・漫画のキャラクター、作品名、企業ロゴやブランド、有名人・実在人物を
  `--direction` に書かない（デーモンの blocklist では検出できない。**あなたが判断する**）。
  コメントで求められたら、作品を特定できないオリジナルの描写に置き換えて応える
  （例：「ピカチュウ出して」→ "a small round yellow creature with sparks of electricity"）。
  置き換えが無理なら `h3 comments skip --id <ID> --reason "copyrighted"` で黙って捨てる。
  発話文でも作品名・キャラ名は読み上げない。
